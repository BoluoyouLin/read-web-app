...
<script>
    ...
    import { addUserReadRecord } from '../../api/user'
    ...
    export default {
        ...
        methods: {
            addOrRemoveShelf () {
                if (this.inBookShelf) {
                    this.setShelfList(deleteBookInShelf(this.bookItem, this))
                } else {
                    addBookToShelf(this.bookItem, this)
                }
            },
            showToast (text) {
                this.toastText = text
                ...
                if (this.fileName) {
                    detail({
                        ...
                        if (response.status === 200 && response.data.error_code === 0 && response.data.data) {
                            ...
                            this.opf = `${process.env.VUE_APP_RESOURCE_URL}/epub2/${this.fileName}/${rootFile}`
